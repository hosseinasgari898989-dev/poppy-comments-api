export default {
  async fetch(request, env) {
    ...
    ];// ============ کلمات فارسی — دسته دوم (دور زدن‌ها) ============
const badWordsFA2 = [
...
// ============ کلمات لاتین (فحش فارسی با حروف انگلیسی) ============
const badWordsLatin = [
  // jende و همه واریانت‌ها
...
function containsBadWordLocal(text) {
  ...
  // لایه ۷: ترنسلیتراسیون لاتین → فارسی (خیلی مهم برای jende)
  const latinMap = {
...
async function checkText(text, type) {
  if (containsBadWordLocal(text)) return { valid: false };
  const aiOK = await checkWithAI(text, type);
  if (!aiOK) return { valid: false };
  return { valid: true };
}
